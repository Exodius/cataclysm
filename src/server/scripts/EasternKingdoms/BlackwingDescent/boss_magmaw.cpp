            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_IGNITION:
                    {
                        ...
                        for(int i=0; i<=1; ++i)
                        {
                            me->GetMap()->GetRandomPointsInTriangle(summonPos, 50, polygonPts[pos][2], polygonPts[pos][3], polygonPts[pos][i]);
                            if(!summonPos.empty())
                                for(std::list<Position>::const_iterator itr = summonPos.begin(); itr != summonPos.end(); ++itr)
                                    me->SummonCreature(NPC_IGNITION, (*itr), TEMPSUMMON_TIMED_DESPAWN, 5000);
                            summonPos.clear();
                        }
