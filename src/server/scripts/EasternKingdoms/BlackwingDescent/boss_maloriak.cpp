...
class boss_maloriak : public CreatureScript
{
    ...
    {
        boss_maloriakAI(Creature * c) : BossAI(c, DATA_MALORIAK) {}
        ...
        void SpellHit(Unit * /*caster*/, const SpellInfo *spell)
        {
            ...
            if (me->HasAura(SPELL_ARCANE_STORM) || (curSpell && curSpell->m_spellInfo->Id == sSpellMgr->GetSpellDifficultyId(SPELL_RELEASE_ABERRATIONS)))
            {
                for (uint8 i = 0; i < 3; ++i)
                {
                    if (spell->Effects[i].Effect == SPELL_EFFECT_INTERRUPT_CAST)
                    {
                        ...
                        me->RemoveAurasDueToSpell(SPELL_ARCANE_STORM);
                    }
                }
            }
        }
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CONSUMING_FLAMES:
                    DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true), SPELL_CONSUMING_FLAMES, true);
                ...
                case EVENT_BITING_CHILL:
                    DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true), SPELL_BITING_CHILL, true);
                    events.ScheduleEvent(EVENT_BITING_CHILL, 15000, EVENT_GROUP_BLUE);
                    break;
                case EVENT_FLASH_FREEZE:
                    DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true), SPELL_FLASH_FREEZE, true);
                ...
                case EVENT_ABSOLUTE_ZERO:
                    DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true), SPELL_ABSOLUTE_ZERO, true);
