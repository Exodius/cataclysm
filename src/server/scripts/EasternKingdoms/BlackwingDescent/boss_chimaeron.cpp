        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CAUSTIC_SLIME:
                    DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, -SPELL_BREAK), SPELL_CAUSTIC_SLIME); 
...
class spell_finkles_mixture : public SpellScriptLoader
{
    class script_impl : public AuraScript
    {
        ...
        void Register()
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(script_impl::CalculateAmount, EFFECT_1, SPELL_AURA_SCHOOL_ABSORB);
            OnEffectAbsorb += AuraEffectAbsorbFn(script_impl::Absorb, EFFECT_1);
        }
    };
