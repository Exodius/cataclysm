        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_CRYSTAL_BARRAGE:
                    DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0), SPELL_CRYSTAL_BARRAGE); 
...
class AreaTrigger_at_rockdoor_break : public AreaTriggerScript
{
    public:
        ...
        AreaTrigger_at_rockdoor_break()
            : AreaTriggerScript("at_rockdoor_break")
        {
        }

        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/)
        {
            if (GameObject* go = GetClosestGameObjectWithEntry(player, GO_ROCKDOOR_BREAK, 85.0f))
