...
public:
    ...
    bool EffectDummyCreature(Unit* /*pCaster*/, uint32 spellId, uint32 effIndex, Creature* creatureTarget)
    {
        ...
        {
            if (creatureTarget->GetEntry() != ENTRY_OWLKIN)
                return true;

            creatureTarget->UpdateEntry(ENTRY_OWLKIN_INOC);

            //set despawn timer, since we want to remove Creature after a short time
            CAST_AI(npc_nestlewood_owlkin::npc_nestlewood_owlkinAI, creatureTarget->AI())->DespawnTimer = 15000;
