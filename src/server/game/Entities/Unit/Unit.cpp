...
void Unit::SendMonsterMove(float NewPosX, float NewPosY, float NewPosZ, uint32 Time, Player* player)
{
    ...
    data << uint8(0);
    data << uint32((GetUnitMovementFlags() & MOVEMENTFLAG_ROOT) ? SPLINEFLAG_FLYING : SPLINEFLAG_WALKING);
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->Id)
    {
		...
		case 92356: // Cataclysm Trinkets proc only if healthBelow  $s1%
        case 92236: 
        case 92234: 
		case 92180:
        {
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
		...
		// Do not remove Stealth on absorb
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
    {
        if(GetEntry() != 35905) // this is hacky for quest "Save Krennan"
        {
...
bool Unit::SetVehicleId(uint32 id)
{
    ...
    uint32 vehicleId = 0;
    if (!CreateVehicleKit(vehicleId, 0))
        return false;
...
void Unit::NearTeleportTo(float x, float y, float z, float orientation, bool casting /*= false*/)
{
    ...
    else
    {
        ...
        if(GetDistance2d(x, y) >= GetMap()->GetVisibilityRange())
        {
        ...
        switch (element)
        {
            ...
            case MSETransportTime2:
                if (hasTransportData && hasTransportTime2)
                    data << m_movementInfo.t_time2;
                break;
            case MSETransportTime3:
                if (hasTransportData && hasTransportTime3)
                    data << m_movementInfo.t_time3;
                break;
            case MSEPitch:
                if (hasPitch)
                    data << m_movementInfo.pitch;
                break;
            case MSEFallTime:
                if (hasFallData)
                    data << m_movementInfo.fallTime;
                break;
            case MSEFallVerticalSpeed:
                if (hasFallData)
                    data << m_movementInfo.j_zspeed;
                break;
            case MSEFallCosAngle:
                if (hasFallData && hasFallDirection)
                    data << m_movementInfo.j_cosAngle;
                break;
            case MSEFallSinAngle:
                if (hasFallData && hasFallDirection)
                    data << m_movementInfo.j_sinAngle;
                break;
            case MSEFallHorizontalSpeed:
                if (hasFallData && hasFallDirection)
                    data << m_movementInfo.j_xyspeed;
                break;
            case MSESplineElevation:
                if (hasSplineElevation)
                    data << m_movementInfo.splineElevation;
                break;
            case MSECounter:
...
void Unit::BuildMovementPacket(ByteBuffer *data) const
{
    switch (GetTypeId())
    {
        case TYPEID_UNIT:
            if (CanFly())
                const_cast<Unit*>(this)->AddUnitMovementFlag(MOVEMENTFLAG_ROOT);
            if (IsVehicle())
                const_cast<Unit*>(this)->AddExtraUnitMovementFlag(GetVehicleKit()->GetExtraMovementFlagsForBase());
            break;
        case TYPEID_PLAYER:
            // remove unknown, unused etc flags for now
            const_cast<Unit*>(this)->RemoveUnitMovementFlag(MOVEMENTFLAG_FALLING_SLOW);
            if (isInFlight())
            {
                ACE_ASSERT(const_cast<Unit*>(this)->GetMotionMaster()->GetCurrentMovementGeneratorType() == FLIGHT_MOTION_TYPE);
                const_cast<Unit*>(this)->AddUnitMovementFlag(MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_FALLING_SLOW);
            }
            break;
        default:
            break;
    ...
    if (Vehicle* pVehicle = GetVehicle())
        if (!HasUnitMovementFlag(MOVEMENTFLAG_ROOT))
            sLog->outError(LOG_FILTER_UNITS, "Unit (GUID: " UI64FMTD ", entry: %u) does not have MOVEMENTFLAG_ROOT but is in vehicle (ID: %u)!",
    ...
    *data << GetOrientation();
    ...
    // 0x00000200
    if (GetUnitMovementFlags() & MOVEMENTFLAG_DISABLE_GRAVITY)
    {
        if (m_vehicle)
    ...
    if (m_movementInfo.flags2 & MOVEMENTFLAG2_INTERPOLATED_TURNING)    // & 0x800, 4.0.6
    {
        ...
        *data << (float)m_movementInfo.j_zspeed;
        ...
        // 0x00001000
        if (GetUnitMovementFlags() & MOVEMENTFLAG_FALLING_FAR)
        {
