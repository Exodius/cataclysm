...
enum SplineFlags
{
    ...
    SPLINEFLAG_FULL_PATH      = 0x40000000,
...
inline void Unit::SendMonsterMoveByPath(Path<Elem, Node> const& path, uint32 start, uint32 end)
{
    ...
    data << uint8(0);
    data << uint32(((GetUnitMovementFlags() & MOVEMENTFLAG_ROOT) || isInFlight())
                   ? (SPLINEFLAG_FLYING|SPLINEFLAG_WALKING|SPLINEFLAG_CATMULL_ROM|SPLINEFLAG_ANIMATION|SPLINEFLAG_FULL_PATH)
                   : SPLINEFLAG_WALKING);
