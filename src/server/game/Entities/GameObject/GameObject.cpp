    ...
    switch (m_lootState)
    {
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            if (sObjectMgr->IsGameObjectForQuests(GetEntry()))
            {
                ...
                Trinity::AnyPlayerInObjectRangeCheck checker(this, 100.0f);
                Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(this, playerList, checker);
                VisitNearbyWorldObject(100.0f, searcher);
                number = std::max<size_t>(playerList.size(), number);
            }
            m_respawnTime = time(NULL) + m_respawnDelayTime / (3 - 2 / number);
