...
void Vehicle::GetSeatPosition(Position* pos, int8 seatId, Unit* passenger)
{
    SeatMap::const_iterator seat;

    if(seatId == -1 && passenger)
    {
        for (seat = Seats.begin(); seat != Seats.end(); ++seat)
            if (seat->second.Passenger == passenger->GetGUID())
                break;
    }
    else
        seat = Seats.find(seatId);
    ...
    if (seat != Seats.end())
    {
        ...
        pos->m_positionX = passenger->GetPositionX() + veSeat->m_attachmentOffsetX;
        pos->m_positionY = passenger->GetPositionY() + veSeat->m_attachmentOffsetY;
        pos->m_positionZ = passenger->GetPositionZ() + veSeat->m_attachmentOffsetZ;
        pos->m_orientation = passenger->GetOrientation();
    }
}
