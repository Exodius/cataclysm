 ...
 * Copyright (C) 2005-2009 HTCore <http://cata.vfire-core.com/>
 *
...
void AuctionHouseBot::addNewAuctionBuyerBotBid (Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
	...
	for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
	{
		...
		if (debug_Out)
		{
			...
			//sLog->outInfo(LOG_FILTER_GENERAL, "AHBuyer: Ammo Type: %u", prototype->InventoryType);
			sLog->outInfo(LOG_FILTER_GENERAL, "-------------------------------------------------");
		}
		...
		else
		{
			...
			uint32 item_template = auction->itemEntry;
...
void AuctionHouseBot::Update()
{
	...
	WorldSession _session(AHBplayerAccount, NULL, SEC_PLAYER, NULL, 3, 0, LOCALE_enUS, 0, 0);
...
void AuctionHouseBot::Initialize()
{
	...
	AHBSide_Override_Side = ConfigMgr::GetBoolDefault("AuctionHouseBot.OverrideSide", false);
	...
	if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION) || AHBSide_Override_Side == true)
	{
		if (AHBSide_Override_Side == true)
            sLog->outError(LOG_FILTER_GENERAL, "Overriding is used");
