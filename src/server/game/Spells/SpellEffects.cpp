    ...
    if (unitTarget && unitTarget->isAlive())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                {
                    // Baron Ashburys - Asphyxoate Debuff
                    case 93424:
                    {
                        damage = unitTarget->GetMap()->IsHeroic() ? CalculatePct(unitTarget->GetHealth(), 20) : 200;
                        break;
                    }
                    // Consumption
                    case 28865:
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            {
                case 80340: // Furious Swipe (Raz the Crazed)
                {
                    m_caster->CastSpell(unitTarget, 80206, true);
                    return;
                }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 66551: // teleports inside (Isle of Conquest)
            ...
            break;
        case 85040: // Teleport to Vashj'ir
            if (Player* target = unitTarget->ToPlayer())
            {
                if (target->GetTeamId() == TEAM_ALLIANCE)
                    m_targets.SetDst(-5310.85f, 3914.42f, 3.224f, 0.0f, 0);
                else
                    m_targets.SetDst(-5671.02f, 3720.07f, 1.915f, 0.0f, 0);
            }
            break;
    }
    ...
    // Remove Shapeshift Auras on Portal usage
            ...
            switch (properties->Type)
            {
                ...
                default:
                {
                    ...
                    for (uint32 count = 0; count < numSummons; ++count)
                    {
                        ...
                        if (m_spellInfo->Id == 76338) // Summon Sun Orb (Rajh)
                            pos.Relocate(pos.m_positionX, pos.m_positionY, pos.m_positionZ + 20.0f);
...
void Spell::EffectLeapBack(SpellEffIndex effIndex)
{
    ...
    if(unitTarget)
       unitTarget->JumpTo(speedxy, speedz, false);
        ...
        m_caster->JumpTo(speedxy, speedz, m_spellInfo->SpellIconID != 1891); //1891: Disengage
}
