...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleAuraSwapSpells,                            //332 SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS implemented in WorldSession::HandleCastSpellOpcode
    &AuraEffect::HandleAuraSwapSpells,                            //333 SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS_2 implemented in WorldSession::HandleCastSpellOpcode
...
void AuraEffect::HandleModUnattackable(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply && (mode & AURA_EFFECT_HANDLE_REAL))
    {
        if (GetId() != 76242 // Ascendant Lord Obsidius' Transformation aura should not drop combat
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit* target, Unit* caster) const
{
    switch (GetSpellInfo()->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch (GetId())
            {
                case 75592: // Divine Reckoning
                    caster->CastSpell(target, 75591, true, NULL, NULL, GetCasterGUID());
                    break;
