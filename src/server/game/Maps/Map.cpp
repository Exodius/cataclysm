...
G3D::Vector3 Map::PositionToVector3(const Position &pos) const
{
    return G3D::Vector3(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ());
}

void Map::GetRandomPointsInTriangle(std::list <Position> &posList, uint8 count, const Position &pos1, const Position &pos2, const Position &pos3)
{
    const G3D::Triangle triangle(PositionToVector3(pos1), PositionToVector3(pos2), PositionToVector3(pos3));
    G3D::Vector3 vec;
    for(int i=0; i<= count; ++i)
    {
        vec = triangle.randomPoint();
        Position pos = {vec.x, vec.y, vec.z, 0.0f};
