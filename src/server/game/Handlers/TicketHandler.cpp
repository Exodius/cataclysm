...
void WorldSession::HandleSubmitBugOpcode(WorldPacket& recvData) {
    uint8 lengthLow, lengthHigh;
    float posY, posZ, posX, posO;
    uint32 mapId;
    uint16 length;
    std::string content;

    recvData >> lengthHigh >> lengthLow;
    length = (lengthHigh << 8) + lengthLow;
    content = recvData.ReadString(length/16);
    recvData >> posY >> posZ >> posX >> mapId >> posO;  

    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_BUG_SUBMISSION);
    stmt->setString(0, content);
    stmt->setUInt64(1, GetPlayer()->GetGUID());
    stmt->setFloat(2, posX);
    stmt->setFloat(3, posY);
    stmt->setFloat(4, posZ);
    stmt->setFloat(5, posO);
    stmt->setUInt32(6, mapId);
